    ...
    if plurk.limited_to is not None and len(plurk.limited_to) == 2:
        ...
        for reply in replies:
            if reply.user_id == plurk_api.user_id:
                ...
                continue
            c.execute("SELECT tweeter, tweet, in_reply_to, userid FROM plurkedtweets WHERE plurk = %s ORDER BY tweet ASC LIMIT 1", plurk.id)
            tweetdata = c.fetchone()
            if tweetdata is None:
                print "Couldn't get any data about the plurk"
                continue
            tweeter, tweet, in_reply_to, user_id = tweetdata
            ...
            c.execute("""INSERT INTO
                plurkedtweets (userid, tweeter, tweet, plurk, `date`, in_reply_to)
                VALUES (%s, %s, %s, %s, NOW(), %s)""",
                (user_id, twitter_nick, status.id, plurk.id, reply_to_id)
            )
